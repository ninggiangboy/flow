...
sequenceDiagram
    ...
    alt Exceeded limit
        ...
        BE->>DB: select id, password_hash, email_verified, secret, status from user where email = ?
        DB-->>BE: user record

        alt User not found or password mismatch
            BE->>Cache: incr login_attempts:{email}/{ip} ttl=60s
            ...
            alt email_verified = false
                ...
                BE-->>FE: return 403 { "error": "Account inactive" }
            else OK
